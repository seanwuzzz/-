      ...
      <section className="bg-slate-800/50 p-5 rounded-2xl border border-slate-700">
        ...
        <div className="relative bg-slate-900 p-3 rounded-lg border border-slate-700 overflow-hidden">
            ...
            <button 
                ...
                className="absolute top-2 right-2 bg-slate-700 hover:bg-slate-600 text-white p-2 rounded-md flex items-center gap-1 text-[10px]"
            >
                {copied ? <Check size={12} className="text-green-400" /> : <Copy size={12} />}
                {copied ? '已複製' : '複製代碼'}
            </button>
        </div>
        <ol className="mt-4 text-[10px] text-slate-400 list-decimal list-inside space-y-1">
            <li>建立一個新的 Google Sheet。</li>
            <li>將分頁命名為 <code>Transactions</code> 與 <code>Prices</code>。</li>
            <li>擴充功能 &gt; Apps Script &gt; 貼上代碼。</li>
            <li>部署 &gt; 新增部署 &gt; Web App &gt; 所有人 (Anyone) 可存取。</li>
        </ol>
      </section>
